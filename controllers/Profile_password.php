...
class Profile_password extends Root_Controller
{
    ...
    private function system_edit()
    {
        if(isset($this->permissions['edit'])&&($this->permissions['edit']==1))
        {
            ...
            $ajax['system_content'][]=array("id"=>"#system_content","html"=>$this->load->view("profile_password/add_edit",$data,true));
            if($this->message)
            {
    ...
    private function system_save()
    {
        ...
        else
        {
            ...
            Query_helper::update($this->config->item('table_setup_user'),$data,array("id = ".$user->user_id));
    ...
    private function check_validation()
    {
        ...
        $user = User_helper::get_user();
        $info=Query_helper::get_info($this->config->item('table_setup_user'),array('id'),array('id ='.$user->user_id,'password ="'.md5($this->input->post('old_password')).'"'),1);
        /*echo '<PRE>';
