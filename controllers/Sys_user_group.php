...
class Sys_user_group extends Root_Controller
{
    ...
    private function system_list()
    {
        if(isset($this->permissions['view'])&&($this->permissions['view']==1))
        {
            ...
            $ajax['status']=true;
            $ajax['system_content'][]=array("id"=>"#system_content","html"=>$this->load->view("sys_user_group/list",$data,true));
            if($this->message)
            {
    ...
    private function system_add()
    {
        if(isset($this->permissions['add'])&&($this->permissions['add']==1))
        {
            ...
            $ajax['status']=true;
            $ajax['system_content'][]=array("id"=>"#system_content","html"=>$this->load->view("sys_user_group/add_edit",$data,true));
            if($this->message)
            {
                $ajax['system_message']=$this->message;
            }
    ...
    private function system_edit($id)
    {
        if(isset($this->permissions['edit'])&&($this->permissions['edit']==1))
        {
            ...
            $data['user_group']=Query_helper::get_info($this->config->item('table_system_user_group'),'*',array('id ='.$group_id),1);
            $data['title']="Edit User Group (".$data['user_group']['name'].')';
            $ajax['status']=true;
            $ajax['system_content'][]=array("id"=>"#system_content","html"=>$this->load->view("sys_user_group/add_edit",$data,true));
            if($this->message)
            {
    ...
    private function system_save()
    {
        ...
        {
            $data=$this->input->post('user_group');
    ...
    private function check_validation()
    {
        ...
        $this->form_validation->set_rules('user_group[name]',$this->lang->line('LABEL_NAME'),'required');
