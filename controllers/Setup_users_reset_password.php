...
class Setup_users_reset_password extends Root_Controller
{
    private  $message;
    public $permissions;
    ...
    public function __construct()
    {
        parent::__construct();
        $this->message="";
        $this->permissions=User_helper::get_permission('Setup_users_reset_password');
        $this->controller_url='setup_users_reset_password';
    ...
    private function system_list()
    {
        ...
        {
            $data['title']="List of Users To Reset password";
            $ajax['status']=true;
            $ajax['system_content'][]=array("id"=>"#system_content","html"=>$this->load->view("setup_users_reset_password/list",$data,true));
            if($this->message)
            {
    ...
    private function system_edit($id)
    {
        if(isset($this->permissions['edit'])&&($this->permissions['edit']==1))
        {
            ...
            $ajax['system_content'][]=array("id"=>"#system_content","html"=>$this->load->view("setup_users_reset_password/add_edit",$data,true));
            if($this->message)
            {
                $ajax['system_message']=$this->message;
            }
    ...
    private function system_save()
    {
        ...
        else
        {
            ...
            Query_helper::update($this->config->item('table_setup_user'),$data,array("id = ".$id));
