...
class User_helper
{
    ...
    {
        $CI = & get_instance();
        $user = $CI->db->get_where($CI->config->item('table_setup_user'), array('user_name' => $username, 'password' =>(md5($password)),'status'=>$CI->config->item('system_status_active')))->row();
        if ($user)
        {
    ...
    public static function get_accessed_sites()
    {
        $CI = & get_instance();
        $user=User_helper::get_user();
        $CI->db->from($CI->config->item('table_setup_users_other_sites').' uos');
